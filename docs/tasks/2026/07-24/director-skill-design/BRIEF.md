...
- 결함 증거와 제안 해법을 분리하고 사용자 승인 계약의 정확한 식별자와 경로를 인계에서 보존하며, 해법을 지시하기 전에 기존 `NEEDS_DECISION` 경계를 적용한다. 사용자 교정과 충돌하는 작업은 낡은 지시를 철회하고 담당자가 변경 중단과 교정된 계약을 확인한 뒤 재개한다.

# Current State

결함과 해법의 분리, 정확한 계약 인계와 영향 작업의 안전한 재개 경계를 한영 실행 계약에 보강했다. Project Legibility 출시 전 검토에서 결정 상향 문구가 기존 계약을 좁힐 수 있음을 확인해, 해법을 지시하기 전에 기존 `NEEDS_DECISION` 경계를 적용하도록 교정했다.

...
- **Background:** 위험한 실제 실행의 안전 관문을 격리된 가역적 검증에도 적용하고, 같은 사실을 여러 증거 표면이 소유하면서 단일 검증 조건마다 전체 검증과 검토가 반복됐다.
- **Decision:** 검증 주기를 승인 주장과 실행 영향에 맞춘다. 위험한 실행에는 실행 전 검토와 승인된 관문을 유지하고, 가역적 작업은 관련 변경을 묶어 검토한 최종 변경분에 필요한 넓은 검증을 수행한다. 승인 사실은 정본 증거 출처 하나가 소유하며 모순되는 독립 증거는 완료를 막는다.
- **Why:** 실제 위험 관문을 유지하면서 작은 수정마다 전체 검증과 리뷰를 반복하는 비용을 줄이고 최종 변경분의 증거 최신성을 보장하기 위해서다.
- **Impact:** 작업 정보는 관련 있을 때만 요구한다. 병렬 작업은 공유 계약, 실행 영향, 의존성, 되돌림 경계와 변경 영역이 독립일 때만 허용하고, 공유 통합은 담당자 한 명이 맡는다. 반복 실패 재정의는 영향받는 작업에만 적용한다.

**2026-07-25**

- **Background:** 검토자가 찾은 결함과 제안 해법을 함께 권한 있는 지시로 취급하고, 사용자 교정 뒤에도 낡은 지시와 잘못 재서술한 계약 경로가 작업자에게 남은 실패가 발생했다.
- **Decision:** 결함 증거와 해법을 분리하고 기존 담당자와 계약 안의 가장 작은 해법을 우선한다. 사용자 승인 계약의 정확한 식별자는 인계에서 그대로 보존하며, 교정과 충돌하는 작업은 지시를 철회하고 담당자가 변경 중단과 새 계약을 확인한 뒤 재개한다.
- **Why:** 검토의 유효한 결함 발견은 살리면서 권한 없는 구조 확장과 교정 전 지시의 지속 실행을 함께 막기 위해서다.
- **Impact:** 승인 계약이나 담당 경계를 바꿔야만 진행할 수 있을 때만 `NEEDS_DECISION`으로 전환한다. 새 구조 유형별 자동 중단, 전역 작업 정지와 별도 상태는 추가하지 않는다.

**2026-07-25**

- **Background:** 출시 전 검토에서 해법 제안의 상향 제한이 기존 `NEEDS_DECISION`의 담당자 권한·프로젝트 영향 조건까지 좁힐 수 있음이 확인됐다.
- **Decision:** 해법을 지시하기 전에 기존 `NEEDS_DECISION` 경계를 적용한다.
- **Why:** 새 조건 목록을 만들지 않고 기존 권한 상향 계약을 보존하면서 권한 없는 해법 확장을 막기 위해서다.
- **Impact:** 사용자 교정의 영향 작업 중단·정확한 계약 인계 경계는 유지되고 기존 결정 상향 계약과의 충돌은 제거된다.

...
- 명시적으로 지정된 director session은 같은 프로젝트 결과의 Codex Goal 하나를 유지한다. Goal은 active continuation을, 짧은 Director State는 현재 행동과 대기를 맡는다.
- Director State는 기존 문서 규칙을 따르되 기본은 `docs/director-state.md`이며, `Goal`, `Now`, `Waiting`, `Constraints`만 현재형으로 덮어쓴다.
...
- 반복된 event나 checkpoint에도 실질적 진전이 없으면 일을 더 추가하기 전에 감독 전략 자체를 점검하고, 기존 조정·복구 수단 중 가장 작고 가역적인 변경 하나를 다음 progress signal과 함께 시험한다.
- 세션 교훈은 authority가 아닌 scoped hypothesis이며, 재사용할 coordination rule은 반복 실패와 forward-test 뒤에만 승격한다.
...
변경은 실제 director session과 forward-test로 검증하고 별도의 release gate를 통과한다.
